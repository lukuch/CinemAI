...
class SklearnClusteringService(ClusteringService):
    ...
    def _best_kmeans_clusters(self, X, weights, ratings, movies, min_k=2, max_k=10):
        ...
        self.logger.info(
            "KMeans clustering completed", clusters=len(clusters), best_k=best_k, silhouette_score=round(best_score, 3)
        )
        return clusters

    def _hdbscan_clusters(self, X, weights, ratings, movies, min_cluster_size=10, min_samples=2, noise_threshold=0.5):
    ...
    def cluster(
        self, embeddings: List[Embedding], ratings: List[float], dates: List[str], movies: List[Movie]
    ) -> List[Cluster]:
        self.logger.info(
        ...
        if n < 100:
            self.logger.info("Using single cluster approach (small dataset)", movies_count=n)
            centroid = np.average(X, axis=0, weights=weights)
            cluster = Cluster(
                centroid=Embedding(list(centroid)), movies=movies, average_rating=np.average(ratings, weights=weights), count=n
            )
            self.logger.info("Single cluster created", movies_count=n, avg_rating=round(cluster.average_rating, 3))
            return [cluster]
        elif n <= 500:
