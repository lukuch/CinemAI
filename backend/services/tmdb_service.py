        ...
        def matches(movie):
            ...
            if 'duration' in filters and filters['duration']:
                if movie['duration'] != filters['duration']:
                    return False
            return True
        filtered = [Movie(**m) for m in movies if matches(m)]
        return filtered

    async def _fetch_and_cache_top_movies(self, total_movies: int = 5000, batch_size: int = 20) -> int:
        movies_per_page = 20
        ...
        async with httpx.AsyncClient() as client:
            for page_start in range(1, total_pages + 1, batch_size):
                page_tasks = [self._fetch_page(client, page) for page in range(page_start, min(page_start + batch_size, total_pages + 1))]
    ...
    async def _fetch_details(self, client, m, genre_map):
        try:
            ...
            return {
                ...
                "countries": [c["iso_3166_1"] for c in details.get("production_countries", [])] if details.get("production_countries") else [],
