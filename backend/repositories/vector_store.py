...
from domain.interfaces import IVectorStoreRepository
from schemas.watch_history import MovieHistoryItem


class PgvectorRepository(IVectorStoreRepository):
    ...
    async def save_user_profile(self, profile: DomainUserProfile):
        ...
        movie_to_index = (
            {self._movie_key(m): i for i, m in enumerate(profile.movies)}
            if profile.movies
            else {}
        )
        clusters_list = [
            {
                "centroid": (
                    ...
                    if hasattr(c.centroid.vector, "tolist")
                    else c.centroid.vector
                ),
        ...
        if db_profile:
            ...
            db_profile = UserProfile(
                user_id=profile.user_id, clusters=clusters_list, movies=movies_list
            )
    ...
    async def get_user_profile(self, user_id: str) -> DomainUserProfile:
        ...
        clusters = [
            Cluster(
                ...
                movies=[
                    movies[i]
                    for i in c.get("movie_indices", [])
                    if i is not None and i < len(movies)
                ],
